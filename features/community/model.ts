...
export type CommunityMessage = { id: string; channelId: string; author: string; authorId: string; color: string; avatarUrl?: string | null; content: string; time: string; attachment?: MessageAttachment };
...
export function messageFromRow(row: MessageRow, profile: Pick<Profile, "display_name" | "accent_color" | "avatar_url">): CommunityMessage {
  const attachment = row.attachment_kind && row.attachment_url && row.attachment_mime && row.attachment_size && row.attachment_name
    ...
    : undefined;
  return { id: row.id, channelId: row.channel_id, author: profile.display_name, authorId: row.author_id, color: profile.accent_color, avatarUrl: profile.avatar_url, content: row.content, time: new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" }).format(new Date(row.created_at)), attachment };
}
