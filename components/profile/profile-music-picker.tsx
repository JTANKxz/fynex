...
export function ProfileMusicPicker({ value, onChange }: { value: SpotifyTrack | null; onChange: (track: SpotifyTrack | null) => void }) {
  ...
  return <section className="profile-music-picker">
    ...
    <form onSubmit={search}><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} maxLength={80} placeholder="Buscar música ou artista" aria-label="Buscar música no Spotify" /><button disabled={loading || query.trim().length < 2}>{loading ? <LoaderCircle className="spin" size={15} /> : "Buscar"}</button></form>
