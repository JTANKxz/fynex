...
export function ProfileForm({ profile, onSaved, onCancel }: { profile: Profile; onSaved?: () => void; onCancel?: () => void }) {
  ...
  return <form action={action} className="profile-form">
    ...
    <label>Cor do card<div className="color-field"><input name="accentColor" type="color" value={color} onChange={(event) => setColor(event.target.value)} /><span>{color}</span></div></label>
