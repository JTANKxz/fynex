...
export function ProfileForm({ profile, onSaved, onCancel }: { profile: Profile; onSaved?: () => void; onCancel?: () => void }) {
  ...
  return <form action={action} className="profile-form">
    ...
    <label>Cor do perfil<div className="profile-color-picker" style={{ "--selected-profile-color": color } as React.CSSProperties}><input name="accentColor" type="color" value={color} onChange={(event) => setColor(event.target.value)} aria-label="Escolher cor do perfil" /><span /></div></label>
