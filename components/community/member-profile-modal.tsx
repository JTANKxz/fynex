...
export type MemberProfile = Pick<Profile, "id" | "username" | "display_name" | "bio" | "avatar_url" | "banner_url" | "accent_color" | "created_at"> & {
  joinedAt?: string;
  online?: boolean;
  isOwner?: boolean;
  roles?: CommunityRole[];
  ...
  return <div className={styles.backdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className={styles.modal} role="dialog" aria-modal="true" aria-label={`Perfil de ${profile.display_name}`} style={{ "--member-accent": profile.accent_color } as React.CSSProperties}>
      ...
      <div className={styles.identity}>
        <div className={styles.avatar} style={profile.avatar_url ? { backgroundImage: `url(${profile.avatar_url})` } : undefined}>{!profile.avatar_url && initials}<i className={profile.online ? styles.online : styles.offline} /></div>
